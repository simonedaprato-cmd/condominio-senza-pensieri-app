...
export default function App() {
  ...
  const notificaVotazioneCompleta = async (pratica, votiAggiornati = []) => {
    try {
      ...
      if ((storicoNotifiche || []).length > 0) return;
      ...
      const { data: aventiDiritto, error: aventiDirittoError } = await supabase
        .from('utenti_condomini')
        .select('email')
      ...
      const emailAventiDiritto = [...new Set((aventiDiritto || [])
        .map((u) => String(u.email || '').toLowerCase().trim())
        .filter(Boolean)
        .filter((email) => email !== String(condominio?.amministratore_email || '').toLowerCase().trim())
      )];
      ...
      const { error: funzioneError } = await supabase.functions.invoke('notifica-votazione-completa', {
        body: payload,
      });

      if (funzioneError) throw funzioneError;
  ...
  const aggiornaVotoCondomino = async (pratica, voto, nota = '') => {
    ...
    await notificaVotazioneCompleta(pratica, votiAggiornati || []);
    await carica();
  };
