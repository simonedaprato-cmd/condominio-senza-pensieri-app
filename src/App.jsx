        ...
        <div className="mx-auto mt-7 max-w-3xl rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 text-center shadow-2xl shadow-black/20">
          ...
          <div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row">
            {pianoNorm === 'premium' ? (
              ...
              <button
                type="button"
                onClick={richiestaInAppuntamento ? () => onOpenHomeIntelligente?.() : undefined}
                disabled={!richiestaInAppuntamento}
                className={`rounded-2xl px-5 py-3 text-sm font-black shadow-xl transition ${richiestaInAppuntamento ? 'bg-emerald-300 text-slate-950 shadow-emerald-950/30 hover:-translate-y-0.5 active:translate-y-0' : 'cursor-not-allowed border border-white/15 bg-white/10 text-white/72 shadow-black/20'}`}
              >
                {richiestaLabel}{richiestaInAppuntamento ? ' → Home Intelligente' : ''}
              </button>
            ) : (
          ...
          <div className="relative flex items-center justify-end gap-2 text-right text-xs font-black uppercase tracking-[0.16em] text-white/80 md:text-sm">
            {isCondominoTopbar ? (
              <button
                type="button"
                onClick={onOpenPiano}
                className="rounded-full border border-white/15 bg-white/10 px-2 py-1 transition hover:bg-white/15 active:scale-95"
                title="Apri scheda piani CSP"
              >
          ...
          return (
            <div key={stato} className="flex h-96 flex-col rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <span className={'rounded-full border px-3 py-1 text-xs font-black ' + badgeClass(stato)}>{String(stato).toLowerCase() === 'appuntamento' ? 'appuntamento fissato' : stato}</span>
...
function GestoreRichiesteUpgradeCspSuite({ richieste = [], condomini = [], onRefresh, onUpdateStato, onFissaAppuntamento, onConvertiPiano, onAnnullaRichiesta }) {
            ...
            return (
              <article key={item.id || `${item.richiedente_email}-${item.created_at}`} className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-slate-950/5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex shrink-0 items-start justify-end lg:min-w-[170px]">
                    <span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] shadow-sm ${statoBadgeClass(stato)}`}>
            ...
            <div className="space-y-4 p-5">
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl bg-slate-50 p-4">
                  ...
                  {richiestaAperta.richiedente_telefono && (
                    <a href={`tel:${String(richiestaAperta.richiedente_telefono || '').replace(/\D/g, '')}`} className="mt-3 inline-flex w-full items-center justify-center rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-emerald-900/20">
                      Chiama
                    </a>
                  )}
                </div>
                <div className="rounded-2xl bg-slate-50 p-4 md:col-span-2">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Richiesta</p>
                  <p className="mt-1 break-all text-sm font-black text-slate-800">{richiestaAperta.richiedente_email || 'Email non indicata'}</p>
                  <p className="mt-2 text-xs font-bold text-slate-500">Da CSP {String(richiestaAperta.piano_attuale || 'base').toUpperCase()} a CSP {String(richiestaAperta.piano_richiesto || 'upgrade').toUpperCase()}</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${statoBadgeClass(richiestaAperta.stato || 'nuova')}`}>{String(richiestaAperta.stato || 'nuova').toLowerCase() === 'appuntamento' ? 'Appuntamento fissato' : richiestaAperta.stato || 'nuova'}</span>
                  </div>
                </div>
              </div>
...
function formatHomeIntelligenteTime(value) {
  if (!value) return '';
...
function PianiCspCondomino({ pianoAbbonamento = 'base', condomini = [], onBackHome, onRichiediUpgrade }) {
  ...
  const cards = [
    ...
    },
    {
      ...
      cta: 'Manifesta interesse per CSP Plus',
    },
    {
      id: 'premium',
      nome: 'CSP Premium',
      ...
      cta: 'Manifesta interesse per CSP Premium',
    },
  ];
  ...
  return (
    <section className="fixed inset-0 z-[175] overflow-y-auto bg-slate-100 p-4 pb-24 md:p-6">
          ...
          return (
            <article key={card.id} className={`rounded-[2rem] border p-5 shadow-sm ${cardClass(card.id)} ${attivo ? 'ring-4 ring-emerald-100' : ''}`}>
              ...
              {card.cta && !attivo && (
                <button
                  type="button"
                  onClick={() => onRichiediUpgrade?.(card.id)}
                  className={`mt-5 w-full rounded-2xl px-4 py-3 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5 ${card.id === 'premium' ? 'bg-amber-500 shadow-amber-900/20' : 'bg-sky-600 shadow-sky-900/20'}`}
                >
                  {card.cta}
                </button>
              )}
              {attivo && (
                <div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">
                  Piano attualmente attivo per questo condominio.
                </div>
              )}
            </article>
          );
...
function HomeIntelligenteCondomino({
  ...
  const documentiApertiStorageKey = `csp_home_intelligente_documenti_aperti_${emailCorrente || 'utente'}_${condominioCorrente?.id || 'all'}`;
  const [documentiAperti, setDocumentiAperti] = useState({});

  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      const saved = window.localStorage.getItem(documentiApertiStorageKey);
      setDocumentiAperti(saved ? JSON.parse(saved) : {});
    } catch {
      setDocumentiAperti({});
    }
  }, [documentiApertiStorageKey]);
  ...
  const isStessoCondominio = (item) => {
    if (!condominioCorrente?.id) return true;
    const id = item?.condominio_id || item?.condominioId || item?.condominio;
    if (!id || Number.isNaN(Number(id))) return true;
    return Number(id) === Number(condominioCorrente.id);
  };

  const giorniDaOggi = (value) => {
    if (!value) return null;
    const data = new Date(value);
    if (Number.isNaN(data.getTime())) return null;
    const giorno = new Date(data.getFullYear(), data.getMonth(), data.getDate());
    return Math.round((giorno - inizioOggi) / 86400000);
  };

  const salvaDocumentoAperto = (key) => {
    const next = { ...(documentiAperti || {}), [key]: new Date().toISOString() };
    setDocumentiAperti(next);
    if (typeof window !== 'undefined') {
      try {
        window.localStorage.setItem(documentiApertiStorageKey, JSON.stringify(next));
      } catch {
        // se localStorage non è disponibile, la bolla sparisce comunque nella sessione corrente
      }
    }
  };
  ...
  const tonoBollaClass = (tono) => {
    if (tono === 'rosso') return 'border-red-100 bg-red-50 hover:border-red-200 hover:bg-red-50 text-red-700';
    if (tono === 'arancio') return 'border-amber-100 bg-amber-50 hover:border-amber-200 hover:bg-amber-50 text-amber-700';
    if (tono === 'viola') return 'border-violet-100 bg-violet-50 hover:border-violet-200 hover:bg-violet-50 text-violet-700';
    if (tono === 'blu') return 'border-sky-100 bg-sky-50 hover:border-sky-200 hover:bg-sky-50 text-sky-700';
  ...
  const statoUpgrade = String(richiestaUpgradeAttiva?.stato || '').toLowerCase();
  const richiestaUpgradeDelCondomino = richiestaUpgradeAttiva
    ? normalizeEmail(richiestaUpgradeAttiva?.richiedente_email || richiestaUpgradeAttiva?.email || richiestaUpgradeAttiva?.created_by || '') === emailCorrente
    : false;

  const bollaUpgrade = richiestaUpgradeAttiva
    ? (() => {
      ...
      if (statoUpgrade === 'appuntamento') {
        ...
        return creaBolla({
          ...
          cta: 'Apri Home →',
          focusId: 'home-upgrade-appuntamento',
          onClick: () => {
            onGoToSection?.('home-intelligente');
            window.setTimeout(() => {
              const target = document.getElementById('home-upgrade-appuntamento');
      ...
      return creaBolla({
        id: `upgrade-${richiestaUpgradeAttiva.id}`,
        priorita: 3,
        tono: richiestaUpgradeDelCondomino ? 'arancio' : 'blu',
        icona: richiestaUpgradeDelCondomino ? '🟠' : '🔵',
        categoria: 'Upgrade CSP',
        titolo: richiestaUpgradeDelCondomino ? 'Richiesta upgrade inviata' : 'Upgrade richiesto da un condòmino',
        sottotitolo: `${primoCondominio} · CSP ${pianoRichiesto}`,
        dettaglio: richiestaUpgradeDelCondomino ? 'Il gestore CSP prenderà in carico la richiesta.' : 'Un condòmino ha già manifestato interesse per un piano superiore.',
        cta: 'Apri piani →',
        onClick: onOpenPiano,
      });
    })()
  ...
  const bolleAssemblee = (segnalazioni || [])
    .filter(isStessoCondominio)
    .filter((item) => item?.data_assemblea)
    .map((item) => {
      const diff = giorniDaOggi(item.data_assemblea);
      if (diff === null || diff < 0 || diff > 30) return null;
      const isOggi = diff === 0;
      const isImminente = diff > 0 && diff <= 7;
      return creaBolla({
        id: `assemblea-${item.id}`,
        priorita: isOggi ? 1 : isImminente ? 2 : 4,
        tono: isOggi ? 'rosso' : isImminente ? 'arancio' : 'viola',
        icona: isOggi ? '🔴' : isImminente ? '🟠' : '🟣',
        categoria: 'Assemblea',
        titolo: isOggi ? 'Assemblea oggi' : isImminente ? 'Assemblea entro 7 giorni' : 'Assemblea convocata',
        sottotitolo: item.titolo || item.descrizione || 'Pratica condominiale',
        dettaglio: `${formatHomeIntelligenteDate(item.data_assemblea)}${item.luogo_assemblea ? ` · ${item.luogo_assemblea}` : ''}`,
        cta: 'Apri →',
        onClick: () => onOpenSegnalazione?.(item),
      });
    })
    .filter(Boolean)
    .slice(0, 3);
  ...
  const prossimiAppuntamenti = [
    ...(segnalazioni || [])
      ...
      .map((item) => ({
        ...
        ora: item.ora_inizio_lavori_presunta || item.ora_pianificazione || '',
        onClick: () => onOpenSegnalazione?.(item),
      })),
    ...(lavoriPrivati || [])
      .filter((item) => item.data_pianificazione)
      .map((item) => ({
        id: `lsp-${item.id}`,
        tipo: 'LSP',
        icona: '🏠',
        ...
        ora: item.ora_pianificazione || '',
        onClick: () => onOpenLavoroPrivato?.(item),
      })),
  ]
    .filter((item) => {
      const data = new Date(item.data);
      return !Number.isNaN(data.getTime()) && data >= new Date(oggi.getFullYear(), oggi.getMonth(), oggi.getDate() - 1);
    })
    .sort((a, b) => new Date(a.data) - new Date(b.data))
    .slice(0, 3);

  const bolleAppuntamenti = prossimiAppuntamenti.map((item) => creaBolla({
    id: `app-${item.id}`,
    priorita: 3,
    tono: 'blu',
    icona: item.icona,
    ...
    dettaglio: `${item.appuntamentoLabel || 'Appuntamento programmato per'} · ${formatHomeIntelligenteDate(item.data)}${item.ora ? ` · ore ${formatHomeIntelligenteTime(item.ora)}` : ''}`,
    cta: 'Apri →',
    onClick: item.onClick,
  }));

  const ultimoReport = [...(reports || [])]
    .filter(isStessoCondominio)
  ...
  const bolleDocumenti = [
    ultimoReport && !documentiAperti?.[reportKey] ? creaBolla({
      ...
      icona: '📄',
      categoria: 'Report',
      titolo: 'Nuovo report disponibile',
      sottotitolo: ultimoReport?.titolo || primoCondominio,
      dettaglio: 'È disponibile un nuovo report da consultare.',
      cta: 'Apri report →',
      onClick: () => {
        salvaDocumentoAperto(reportKey);
        ultimoReport?.file_url ? onOpenReport?.(ultimoReport) : onGoToSection?.('report');
      },
    }) : null,
    ultimaRivista && !documentiAperti?.[rivistaKey] ? creaBolla({
      id: rivistaKey,
      priorita: 4,
      tono: 'verde',
      icona: '📰',
      ...
      dettaglio: 'La nuova uscita è pronta per essere sfogliata.',
      cta: 'Sfoglia →',
      onClick: () => {
        salvaDocumentoAperto(rivistaKey);
        ultimaRivista?.file_url ? onOpenRivista?.(ultimaRivista) : onGoToSection?.('rivista');
      },
    }) : null,
  ].filter(Boolean);
    ...
    <section className="space-y-5 pb-36 md:pb-8">
      <div className="relative overflow-hidden rounded-[2.4rem] bg-slate-950 p-6 text-white shadow-2xl shadow-slate-950/20 md:p-8">
        ...
        <button type="button" onClick={onOpenPiano} className={`relative mt-6 w-full rounded-[2rem] border p-4 text-left shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl ${pianoCls}`}>
          ...
          <div className="mt-2 flex items-center justify-between gap-3">
            <div>
              <p className="text-2xl font-black">{pianoLabel}</p>
              <p className="mt-1 text-xs font-bold opacity-75">Apri la scheda dei piani e scopri i servizi disponibili.</p>
            </div>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            {bolleAttenzione.map((item) => (
              <button key={item.id} id={item.focusId || undefined} type="button" onClick={item.onClick} className={`rounded-[1.5rem] border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md ${tonoBollaClass(item.tono)}`}>
                <div className="flex items-start gap-3">
                  <span className="text-2xl">{item.icona}</span>
                  <div className="min-w-0 flex-1">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] opacity-70">{item.categoria}</p>
                  ...
                  <span className="shrink-0 text-xs font-black uppercase tracking-[0.12em] opacity-70">{item.cta}</span>
                </div>
              </button>
            ))}
          </div>
...
export default function App() {
  ...
  const apriFullscreenPianiCsp = () => {
    setStatusMessage('');
    setShowPianiCspExperience(true);
  };

  const richiediUpgradeCspCondomino = async (pianoRichiesto) => {
    const pianoPulito = normalizzaPianoAbbonamento(pianoRichiesto);
    if (!['plus', 'premium'].includes(pianoPulito)) return;

    const condominioId = Number(condominioIdPerAbbonamento || condominiVisibili?.[0]?.id || 0);
    const condominio = condomini.find((item) => Number(item.id) === condominioId) || condominiVisibili?.[0] || null;
    const condominioNome = condominio?.nome || condominio?.denominazione || userProfile?.condominio || 'il tuo condominio';

    if (!condominioId) {
      alert('Non riesco a identificare il condominio collegato alla richiesta.');
      return;
    }
    ...
    if (richiestaGiaAttiva) {
      const stato = String(richiestaGiaAttiva.stato || '').toLowerCase();
      if (stato === 'appuntamento') {
        setShowPianiCspExperience(false);
        setCondominoSection('home-intelligente');
    ...
    const pianoLabel = pianoPulito.toUpperCase();
    const conferma = window.confirm(`Vuoi manifestare interesse per CSP ${pianoLabel} per ${condominioNome}?

Il gestore riceverà una richiesta dedicata e potrà fissare un appuntamento visibile nella Home Intelligente.`);
    if (!conferma) return;
    ...
    const payload = {
      ...
      condominioId,
      condominioNome,
      famiglie: getNumeroFamiglieCondominio(condominio),
      richiedenteEmail: utente?.email || userProfile?.email || '',
      richiedenteNome: userProfile?.nome || userProfile?.nome_cognome || '',
      origine: 'home_intelligente_condomino',
    };

    try {
      const { data, error } = await supabase.functions.invoke('request-upgrade-csp', { body: payload });
      if (error) throw error;
      alert(data?.message || `Interesse per CSP ${pianoLabel} inviato al gestore.`);
      setShowPianiCspExperience(false);
      setStatusMessage(`Interesse per CSP ${pianoLabel} inviato al gestore.`);
      await carica();
    } catch (error) {
      console.error('[CSP request-upgrade condòmino] errore', error);
      alert('Errore invio interesse piano: ' + (error?.message || 'sconosciuto'));
    }
  };
  ...
  const apriNotificaCentro = (notifica) => {
    ...
    if (tipo.includes('upgrade_csp') || tipo.includes('upgrade csp')) {
      if (['condominio', 'condomino'].includes(ruoloNormalizzato)) {
        setCondominoSection('home-intelligente');
        window.setTimeout(() => {
          const target = document.getElementById('home-upgrade-appuntamento');
          target?.scrollIntoView?.({ behavior: 'smooth', block: 'center' });
        }, 160);
      } else if (ruoloNormalizzato === 'gestore') {
        setGestoreSection('richieste-upgrade-csp');
  ...
  const annullaRichiestaUpgradeCsp = async (richiesta) => {
  ...
  const convertiRichiestaUpgradeCsp = async (richiesta, pianoFinale) => {
    const piano = normalizzaPianoAbbonamento(pianoFinale || richiesta?.piano_richiesto || 'plus');
    if (!richiesta?.condominio_id || !['plus', 'premium'].includes(piano)) return;
    const condominio = (condomini || []).find((item) => Number(item.id) === Number(richiesta.condominio_id));
    const famiglie = getNumeroFamiglieCondominio(condominio) || Number(richiesta?.famiglie || 0) || 1;
    const config = PIANI_ABBONAMENTO[piano] || PIANI_ABBONAMENTO.plus;
    const ricavoMensile = famiglie * Number(config.costo || 0);

    await creaContratto({
      condominio_id: Number(richiesta.condominio_id),
      piano,
      famiglie,
      costo_unitario: config.costo,
      ricavo_mensile: ricavoMensile,
      ricavo_annuo: ricavoMensile * 12,
      gruppo_whatsapp_attivo: config.whatsapp,
      app_attiva: config.app,
    });
  ...
  return (
    <div className="min-h-screen max-w-full overflow-x-hidden bg-slate-50 px-3 py-4 md:p-6">
      ...
      {showPianiCspExperience && ['condominio', 'condomino'].includes(ruoloNormalizzato) && (
        <PianoCspExperienceModal
          piano={pianoAbbonamentoCorrente}
          richiestaUpgradeAttiva={richiestaUpgradeCspAttivaCorrente}
          currentUserEmail={utente?.email || userProfile?.email || ''}
          onClose={() => setShowPianiCspExperience(false)}
          onOpenHomeIntelligente={() => {
            setShowPianiCspExperience(false);
            setCondominoSection('home-intelligente');
          }}
          onOpenUpgrade={() => {
      ...
      {showUpgradeCspRequest && ['condominio', 'condomino'].includes(ruoloNormalizzato) && (
      ...
      {upgradeCspConfermato && ['condominio', 'condomino'].includes(ruoloNormalizzato) && (
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-3 overflow-x-hidden">
        ...
        {ruoloNormalizzato === 'gestore' && gestoreSection === 'richieste-upgrade-csp' && (
          <GestoreRichiesteUpgradeCspSuite
            ...
            onConvertiPiano={convertiRichiestaUpgradeCsp}
        ...
        {['condominio', 'condomino'].includes(ruoloNormalizzato) && condominoSection === 'home-intelligente' && (
          <HomeIntelligenteCondomino
            userProfile={userProfile}
            condomini={condominiVisibili}
            pianoAbbonamento={pianoAbbonamentoCorrente}
            segnalazioni={segnalazioniVisualizzate}
            ...
            reports={reportVisibili}
            riviste={rivisteCondominio}
            richiesteUpgradeCsp={richiesteUpgradeCsp.filter((item) => {
